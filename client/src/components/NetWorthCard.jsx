...
const NetWorthCard = ({ budget = 0, expense = 0 }) => {
    const netWorth = budget - expense;
    ...
    return (
        <div className="p-6 rounded-2xl bg-gradient-to-b from-slate-100/60 to-slate-200/60 dark:from-[#0c0f1c] dark:to-[#1a1d2e] shadow-xl border border-slate-200 dark:border-slate-700 transform hover:scale-[1.05] transition-all duration-300 ease-in-out">
            ...
            <p className={`text-3xl font-bold ${netWorth >= 0 ? "text-purple-600" : "text-red-500"}`}>₹{netWorth.toLocaleString()}</p>
            <p className="text-sm mt-2 text-slate-600 dark:text-slate-400">(Budget - Expenses)</p>
        </div>
    );
};
